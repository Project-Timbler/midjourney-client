...
exports.DiscordMessage = exports.extractPrompt = void 0;
//TYPES IMPORTS
...
function extractPrompt(content, id) {
    if (!content) {
        return undefined;
    }
    const result = {
    ...
    if (id === "936929561302675456") {
        deps_js_1.logger.warn(`Failed to extract prompt data from: ${deps_js_1.pc.yellow(content)}`);
        deps_js_1.logger.warn(`Extra data:"${deps_js_1.pc.yellow(extra)}"`);
    }
}
...
class DiscordMessage {
    constructor(client, source) {
        ...
        if (source.interaction) {
            ...
            if (name === "describe") {
                ...
                deps_js_1.logger.info("interaction Name: ", name, this.prompt);
    ...
    get parentInteraction() {
        ...
        if (this.components && this.components.length) {
            ...
            console.error(`FIXME: can not Identify signature ${deps_js_1.pc.green(sig1)} in message: ${deps_js_1.pc.green(this.id)}`);
        }
        return "";
    }
    ...
    canReroll() {
        try {
            return this.getComponents(utils_js_1.REROLL);
        }
        catch (_) {
            return null;
        }
    ...
    canUpscale(id) {
        ...
        try {
            ...
            if (id) {
                if (!c.disabled && c.style !== deps_js_1.ButtonStyle.Primary) {
            ...
            if (id && (id > 4 || id < 0)) {
                deps_js_1.logger.warn(`You asked for a image id out of bound [1,2,3,4]`);
            }
            return null;
        }
    }
    ...
    reroll(progress) {
        const comp = this.getComponents(utils_js_1.REROLL);
        deps_js_1.logger.info(`${comp.custom_id} Reroll will be generated`);
    ...
    upscale(id, progress) {
        ...
        deps_js_1.logger.info(`${comp.custom_id} Upscale will be generated`);
        return __classPrivateFieldGet(this, _DiscordMessage_client, "f").callCustomComponents(this.id, comp, progress);
    }
    variant(id, progress) {
        const comp = this.getComponents(`V${id}`, "Make Variations");
        deps_js_1.logger.info(`${comp.custom_id} Variant will be generated`);
        return __classPrivateFieldGet(this, _DiscordMessage_client, "f").callCustomComponents(this.id, comp, progress);
    }
    async refresh() {
    ...
    async download(attachementId, dest) {
        ...
        try {
            ...
            if (stats.isDirectory) {
                const destFile = deps_js_1.path.join(dest, att.filename);
                deps_js_1.logger.info(`downloading ${att.url} to ${destFile}`);
                return await (0, utils_js_1.downloadFileCached)(att.url, destFile);
            }
            throw Error(`download abort, ${dest} is an existing file`);
        }
        catch (_) {
            if (deps_js_1.path.basename(dest).includes(".")) {
                return await (0, utils_js_1.downloadFileCached)(att.url, dest);
            }
            else {
